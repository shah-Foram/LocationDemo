package com.locationdemo.service;
...
import com.locationdemo.activity.MainActivity;
...
private void addLatLongInFirebaseDatabase() {
        Long tsLong = System.currentTimeMillis()/1000;
        String ts = tsLong.toString();

//        LocationModel locationModel = new LocationModel();
        UserModel userModel = new UserModel(latitude,longitude,ts);
        AndroidApp.getInstance().getmDatabase().child("users").child(Preference.getInstance().getUserId()).child("location").child(AndroidApp.getInstance().getmDatabase().push().getKey()).setValue(userModel);
